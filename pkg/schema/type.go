...
func (a *ArrayType) CheckType(node yamlmeta.TypeWithValues) (chk yamlmeta.TypeCheck) {
	//TODO: Is this ever called? Should it be called in Array.Check?
...
func typeToString(value interface{}) string {
	// TODO: this functions is extremely similar to yamlmeta.typeToString
...
func (t *DocumentType) AssignTypeTo(typeable yamlmeta.Typeable) (chk yamlmeta.TypeCheck) {
	...
	if ok || doc.Value == nil {
		if t.ValueType != nil {
			...
			chk.Violations = append(chk.Violations,
				fmt.Errorf("data values found in %s, but schema by %s has none defined\n"+
					"(hint: define required keys in the schema, or do not enable the schema feature)",
					typeableChild.GetPosition().FileString(), t.Position.FileString()))
		}
	} else {
